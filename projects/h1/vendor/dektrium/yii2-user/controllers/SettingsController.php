...
class SettingsController extends Controller
{
        ...
        return [
            ...
            'access' => [
                ...
                'rules' => [
                    [
                        'allow'   => true,
                        'actions' => ['profile', 'account', 'networks', 'disconnect', 'delete','mycourse'],
                        'roles'   => ['@'],
                    ],
                    [
                        'allow'   => true,
    ...
    $courses =  $query->select(['page.name', 'page.id','user_course.sale'])->from('user_course')
            ->join('LEFT JOIN', 'page', 'page.id = user_course.id_course')->all();*/
        ...
        $courses = $query->select(['user.id',
            ...
            'orders.paid','orders.start_date as date','orders.url','orders.price','orders.id as id_order'])
            ->from('orders')
            ->join('LEFT JOIN', 'user', 'user.id = orders.user_id')
            ->where(['user.id' => \Yii::$app->user->id])
            ->andWhere(['>','orders.date' , '2019-10-15 00:00:00'])
            ->andWhere(['in','orders.id', $autopay_details])->all();
            //->createCommand()->getRawSql();



        $result = array_reduce($courses, function($c, $v){
            $c[ $v['course']][] = $v;
            return $c;
        }, array());
