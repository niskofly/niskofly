...
/* note: Отображение блоков */
/* todo: Проверить правильность отображения на большой выборке */
...
foreach ($includeComponent as $includeComponentItemCode => $includeComponentItemData) {
  if ($includeComponentItemCode) {
    $filterIds = [];

    $propertiesElement = CIBlockElement::GetProperty(CATALOG_ID, $elementId, ["ID" => "ASC"], ["CODE" => $includeComponentItemCode]);
    while ($propertyElement = $propertiesElement->GetNext())
      $filterIds[] = $propertyElement['VALUE'];

    global $arrFilter;
    $arrFilter['=ID'] = $filterIds;
  }
  ...
  $intSectionID = $APPLICATION->IncludeComponent(
    ...
    ".product_offers",
    [
      "NAME_BLOCK" => $includeComponentItemData['NAME'],
      "IBLOCK_TYPE" => $arParams["IBLOCK_TYPE"],
      "IBLOCK_ID" => $arParams["IBLOCK_ID"],
      "ELEMENT_SORT_FIELD" => $includeComponentItemData['SORT_FIELD'],
      "ELEMENT_SORT_ORDER" => $includeComponentItemData['SORT_ORDER'],
...
/* info: Временно скрыто
$arParams['BIG_DATA_RCM_TYPE'] = 'personal';
$APPLICATION->IncludeComponent(
  "bitrix:catalog.bigdata.products",
  "",
  array(
    "TITLE" => "Вас может заинтересовать это",
